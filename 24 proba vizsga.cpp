...
void feladat_6()
{
    ...
    for(int i=0;i<n;i++)
    {
        ...
        for(int j=0;j<n;j++)
        {
            if(tomb[j].nap==7 && tomb[i].sorszam==i+1)
            {
...
void feladat_8()
{
    int cnap,ora,cperc,ido=0;bool igaze=false,ebedszunete=false;
    ...
    }else
    {
        ...
        while(i<n && igaze==false)
        {
            for(int j=0;j<n;j++)
            {
                if(tomb[j].nap==cnap && tomb[j].sorszam==i+1)
                {
                    ido=tomb[j].perc+ido;
                }
                if(ido>cperc)
